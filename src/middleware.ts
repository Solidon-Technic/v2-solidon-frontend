...
async function getCountryCode(
  ...
  try {
    ...
    if (process.env.NODE_ENV === "development") {
      console.error(
        "Middleware.ts: Error getting the country code. Did you set up regions in your Admin panel and define a MEDUSA_BACKEND_URL environment variable? Note that the variable is no longer named NEXT_PUBLIC_MEDUSA_BACKEND_URL."
      )
    }
  }
}
...
export async function middleware(request: NextRequest) {
  ...
  if (!urlHasCountryCode && countryCode) {
    ...
    return new NextResponse(
      "No valid regions configured. Please set up regions with countries in your Admin panel.",
      { status: 500 }
    )
  }
